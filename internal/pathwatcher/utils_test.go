...
import (
	...
	"github.com/aserto-dev/runtime/internal/storage/mock"
...
func TestWatchPaths(t *testing.T) {
	...
	test.WithTempFS(fs, func(rootDir string) {
		paths, err := pathwatcher.GetWatchPaths([]string{"prefix:" + rootDir + "/foo"})
		if err != nil {
...
func TestProcessWatcherUpdateForRegoVersion(t *testing.T) {
	...
	test.WithTempFS(files, func(rootDir string) {
		...
		err = pathwatcher.ProcessWatcherUpdateForRegoVersion(t.Context(), regoVersion, paths, "", store, filter, false, false, f)
		if err != nil {
			t.Fatalf("Unexpected error: %v", err)
		}
	})
