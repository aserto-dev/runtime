...
import (
	...
	"github.com/aserto-dev/runtime/internal/file/archive"
			...
			t.Run(tc.note, func(t *testing.T) {
				test.WithTestFS(tc.fs, useMemoryFS, func(rootDir string, fsys fs.FS) {
					...
					err := storage.Txn(ctx, store, storage.WriteParams, func(txn storage.Transaction) error {
						loaded, err := initload.LoadPaths(paths, nil, tc.asBundle, nil, false, true, false, nil, fsys)
						if err != nil {
							return err
						}
	...
	for _, tc := range tests {
		t.Run(tc.note, func(t *testing.T) {
			test.WithTempFS(tc.folderContent, func(rootDir string) {
				...
				loaded, err := initload.LoadPaths(paths, nil, tc.asBundle, nil, true, false, false, nil, nil)
				if err != nil {
...
func TestWalkPaths(t *testing.T) {
	...
	test.WithTempFS(files, func(rootDir string) {
		...
		// bundle mode
		loaded, err := initload.WalkPaths(paths, nil, true)
		if err != nil {
			t.Fatalf("Unexpected error: %s", err)
		}
		...
		// non-bundle mode
		loaded, err = initload.WalkPaths(paths, nil, false)
		if err != nil {
			t.Fatalf("Unexpected error: %s", err)
		}
...
func TestLoadPathsBundleModeWithFilter(t *testing.T) {
	...
	test.WithTempFS(files, func(rootDir string) {
		...
		// bundle mode
		loaded, err := initload.LoadPaths(paths, func(abspath string, info os.FileInfo, depth int) bool {
